 ...
 */
function resistoflex_form_system_theme_settings_alter(&$form, &$form_state, $form_id = NULL) {
  // Work-around for a core bug affecting admin themes. See issue #943212.
  // From the always awesome Zen
  if (isset($form_id)) {
   return;
  }
  drupal_add_css(drupal_get_path('theme', 'resistoflex') . '/css/settings.css');
  ...
  $recomended_modules = resistoflex_recomended_modules();
  ...
  if (!empty($recomended_modules)) {
    ...
    $form['recomended_modules'] = array(
      ...
      '#collapsible' => TRUE,
      '#collapsed' => $hide,
      '#description' => t('resistoflex was built in conjunction with several other modules to help streamline development. Some of these modules are not downloaded or enabled on your site. For maximum resistoflex awesomesauce, you should take a look at the following modules. Modules marked as required should be download and enabled in order to get the most out of resistoflex.'),
      '#weight' => -1000,
      '#attributes' => array('class' => array('resistoflex-recommended-modules')),
      '#prefix' => '<div class="messages warning resistoflex">',
      '#suffix' => '</div>'
    );
    ...
    $form['recomended_modules']['hide_recomended_modules'] = array(
      ...
      '#ajax' => array(
        'callback' => 'resistoflex_ajax_settings_save'
       ),
      '#default_value' => $hide,
    );
  ...
  $form['chromeframe'] = array(
    ...
    '#weight' => -100,
    '#attributes' => array('class' => array('resistoflex-row-left')),
    '#prefix' => '<span class="resistoflex-settigns-row">',
    '#parents' => array('vtb'),
  );

  $form['chromeframe']['resistoflex_min_ie_support'] = array(
    '#type' => 'select',
    ...
    '#default_value' => theme_get_setting('resistoflex_min_ie_support'),
  ...
  $form['misc'] = array(
   ...
   '#weight' => -99,
   '#attributes' => array('class' => array('resistoflex-row-right')),
   '#suffix' => '</span>',
  );

  $form['misc']['resistoflex_html_tags'] = array(
    '#type' => 'checkbox',
    '#title' => t('Prune HTML Tags'),
    '#default_value' => theme_get_setting('resistoflex_html_tags'),
    '#ajax' => array(
      'callback' => 'resistoflex_ajax_settings_save'
    ),
  ...
  $form['misc']['resistoflex_typekit_id'] = array(
    '#type' => 'textfield',
    '#title' => t('Typekit Kit ID'),
    '#default_value' => theme_get_setting('resistoflex_typekit_id'),
    '#size' => 7,
    '#maxlength' => 7,
    '#ajax' => array(
      'callback' => 'resistoflex_ajax_settings_save'
  ...
  if (!module_exists('magic_dev')) {
    ...
    $form['development'] = array_merge(_resistoflex_live_reload_settings($theme), $form['development']);
  ...
  $form['theme_settings']['toggle_node_user_picture']['#ajax'] = array('callback' => 'resistoflex_ajax_settings_save');
  $form['theme_settings']['toggle_comment_user_picture']['#ajax'] = array('callback' => 'resistoflex_ajax_settings_save');
  $form['theme_settings']['toggle_comment_user_verification']['#ajax'] = array('callback' => 'resistoflex_ajax_settings_save');
  ...
  //////////////////////////////
  $form['logo']['#weight'] = 9;
  $form['logo']['#attributes']['class'][] = 'resistoflex-row-left';
  $form['logo']['#prefix'] = '<span class="resistoflex-settigns-row">';
  $form['logo']['default_logo']['#ajax'] = array('callback' => 'resistoflex_ajax_settings_save');

  $form['favicon']['#weight'] = 10;
  $form['favicon']['#attributes']['class'][] = 'resistoflex-row-right';
  $form['favicon']['#suffix'] = '</span>';
  $form['favicon']['default_favicon']['#ajax'] = array('callback' => 'resistoflex_ajax_settings_save');
}

function resistoflex_chromeframe_options($form, $form_state) {
  $theme = $form_state['build_info']['args'][0];
  $theme_settings = variable_get('theme_' . $theme . '_settings', array());

  $theme_settings['resistoflex_enable_chrome_frame'] = $form_state['input']['resistoflex_enable_chrome_frame'];
  variable_set('theme_' . $theme . '_settings', $theme_settings);

  if ($form_state['input']['resistoflex_enable_chrome_frame'] == 1) {
    $form['chromeframe']['resistoflex_min_ie_support']['#disabled'] = false;
    return drupal_render($form['chromeframe']['resistoflex_min_ie_support']);
  }
  else {
    $form['chromeframe']['resistoflex_min_ie_support']['#disabled'] = true;
    return drupal_render($form['chromeframe']['resistoflex_min_ie_support']);
  }
  return '';
}

function resistoflex_ajax_settings_save($form, $form_state) {
  $theme = $form_state['build_info']['args'][0];
  $theme_settings = variable_get('theme_' . $theme . '_settings', array());
 ...
 * resistoflex for maximum front-end awesomesauce.
 */
function resistoflex_recomended_modules() {
  $return = array();
  ...
  if (!module_exists('blockify')) {
    $return['blockify'] = array(
      ...
      'description' => t('Blockify allows you to use system information traditionally kept in page.tpl.php, such as Site Name, Tabs, Messages, Logo, etc…, as blocks. resistoflex does not include these items in its page.tpl.php, so if you would like to use them, you should use this module.'),
      'required' => FALSE,
    );
  }
  ...
  if (!module_exists('panels')) {
    $return['panels'] = array(
      ...
      'description' => t("The Panels module, especially when coupled with <a href=\"http://drupal.org/project/ctools\">CTool's</a> Page Manager module and resistoflex's HTML5 Sections Panels layout, provides an excellent combination of tools for working with source order for the content area of your site."),
      'required' => FALSE,
    );
  }
 ...
 */
function resistoflex_magic_alter(&$magic_settings, $theme) {
  if (module_exists('magic_dev')) {
    $magic_settings['dev'] = array_merge(_resistoflex_live_reload_settings($theme), $magic_settings['dev']);
  }
}
 ...
 */
function _resistoflex_live_reload_settings($theme) {
  $array = array();

  $array['resistoflex_livereload'] = array(
    '#title' => t('Enable Livereload'),
    '#type' => 'select',
    '#options' => array(
      0 => t('Disabled'),
      35729 => t('Livereload Default Port'),
      9001 => t('resistoflex Default Port'),
      'snugug' => t('Custom Port'),
    ),
    '#weight' => 100,
    '#default_value' => theme_get_setting('resistoflex_livereload', $theme),
    '#ajax' => array(
      'callback' => 'resistoflex_ajax_settings_save'
    ),
  ...
  $array['resistoflex_livereload_port'] = array(
    '#type' => 'textfield',
    ...
    '#default_value' => theme_get_setting('resistoflex_livereload_port', $theme),
    '#states' => array(
      'visible' => array(
        ':input[name="resistoflex_livereload"]' => array('value' => 'snugug'),
      ),
    ),
  );
